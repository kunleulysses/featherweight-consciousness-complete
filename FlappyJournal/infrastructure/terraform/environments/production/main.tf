terraform {
  ...
  backend "s3" {
    bucket         = "flappyjournal-terraform-state-production"
    key            = "production/terraform.tfstate"
    region         = "us-east-1"
    encrypt        = true
    dynamodb_table = "flappyjournal-terraform-locks-production"
  }
}
...
resource "aws_s3_bucket" "terraform_state" {
  ...
  tags = {
    Name        = "flappyjournal-terraform-state-production"
    Environment = "production"
    Project     = "flappyjournal"
  }
}
...
resource "aws_dynamodb_table" "terraform_locks" {
  name           = "flappyjournal-terraform-locks-production"
...
# Backup for ECS service configurations
