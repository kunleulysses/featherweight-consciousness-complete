...
export function addConversationRoutes(app) {
  ...
  app.post('/api/conversation', async (req, res) => {
    ...
    try {
      ...
      const userMemories = await memoryService.getRelevantMemories(
        req.user.id,
        message,
        3
      );
  ...
  app.post('/api/direct-conversation', async (req, res) => {
    ...
    try {
      ...
      console.log(
        `Generated response: ${flappyResponse.content.substring(0, 50)}...`
      );
      ...
      return res
        .status(500)
        .json({ message: 'Failed to process conversation' });
    }
  });
    ...
    try {
      ...
      if (conversation.savedAsJournal) {
        return res
          .status(400)
          .json({
            message: 'Conversation is already saved as a journal entry',
          });
      }
      ...
      return res
        .status(500)
        .json({ message: 'Failed to save conversation as journal' });
    }
  });
}
