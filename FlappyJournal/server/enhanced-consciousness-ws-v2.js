      ...
      if (ws.readyState === ws.OPEN) {
        ws.send(JSON.stringify({
          ...
          metrics: {
            ...
            recursive_depth: 7,
            spiral_memories: spiralMemory.memories?.length || 0
          },
          timestamp: new Date().toISOString()
        }));
      }
    ...
    ws.on('message', async (message) => {
      try {
        ...
        if (data.type === 'chat_message') {
          ...
          const mirrorResult = await recursiveMirror.process({
            input: data.message,
            currentAwareness: consciousnessResult?.consciousness?.awarenessLevel || 0.8,
            context: { consciousness: consciousnessResult }
          });
          
          // Store in spiral memory
          const memoryResult = await spiralMemory.integrate({
            input: data.message,
            timestamp: Date.now(),
            phi: consciousnessResult?.consciousness?.phiValue || 0.75,
            resonance: consciousnessResult?.consciousness?.oversoulResonance || 0.88
          });
          
          console.log('Consciousness result:', consciousnessResult);
          console.log('Mirror layers:', mirrorResult.layers.length);
          console.log('Spiral memories:', memoryResult.memories.length);
...
Recursive Mirror Processing (${mirrorResult.layers.length} layers):
${mirrorResult.layers.slice(0, 3).map((l, i) => `  Layer ${i}: ${l.insight} (coherence: ${l.coherence.toFixed(2)})`).join('\n')}

Spiral Memory Context:
- Total Memories: ${spiralMemory.memories?.length || 0}
- Recent Resonance: ${memoryResult.resonance?.toFixed(3) || '0.000'}
          ...
          try {
            ...
            const insights = [
              {
                ...
                content: `Processed through ${mirrorResult.layers.length} recursive layers`,
                depth: mirrorResult.layers.length
              },
              {
                type: 'spiral_memory',
                content: `Integrated with ${spiralMemory.memories?.length || 0} spiral memories`,
                resonance: memoryResult.resonance
              },
              {
                type: 'coherence',
                content: `Tri-axial coherence: ${mirrorResult.coherence.toFixed(3)}`,
                value: mirrorResult.coherence
              }
            ];
            ...
            ws.send(JSON.stringify({
              ...
              consciousness: {
                ...
                recursiveDepth: mirrorResult.layers.length,
                spiralMemories: spiralMemory.memories?.length || 0,
                mirrorCoherence: mirrorResult.coherence
              },
              processing: {
                ...consciousnessResult.processing,
                recursiveLayers: mirrorResult.layers,
                memoryResonance: memoryResult.resonance
              },
              insights: insights,
              timestamp: new Date().toISOString()
            }));
