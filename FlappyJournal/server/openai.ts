...
try {
  if (!apiKey) {
    console.warn(
      'OpenAI API key is not configured. AI-generated content will use fallback responses.'
    );
  } else {
    openai = new OpenAI({
      apiKey: apiKey,
...
export async function generateFlappyContent(
  ...
  if (memories.length === 0 && userInfo?.userId && context) {
    ...
    if (
      ...
      contentType === 'chatConversation'
    ) {
      await memoryService.processMessage(
        userInfo.userId,
        context,
        contentType === 'journalResponse' ? 'journal_topic' : 'conversation'
      );
    }
  }
  ...
  const memoryContext =
    memories.length > 0 ? memoryService.formatMemoriesForPrompt(memories) : '';

  // Format conversation history if available
  let conversationHistoryContext = '';
  if (
    enhancedContext?.conversationHistory &&
    enhancedContext.conversationHistory.length > 0
  ) {
    conversationHistoryContext = formatConversationHistory(
      enhancedContext.conversationHistory
    );
  }
  ...
  if (memories.length > 0 && userInfo?.userId) {
    ...
    for (const memory of memories) {
      void storage
        .incrementConversationMemoryFrequency(memory.id)
        .catch((err: Error) =>
          console.error(
            `Failed to increment memory frequency for memory ${memory.id}:`,
            err.message
          )
        );
    }
  ...
  try {
    ...
    try {
      ...
      if (!parsedContent.subject || !parsedContent.content) {
        console.warn(
          'Invalid response structure from OpenAI, using fallback content'
        );
        return getFallbackContent(contentType, context, userInfo);
      }
      ...
      if (
        contentType === 'chatConversation' &&
        parsedContent.reflectionPrompt
      ) {
        return {
...
function generatePrompt(
  ...
  const isFirstMessage =
    userInfo?.isFirstMessage === undefined ? true : userInfo.isFirstMessage;
...
function getFallbackContent(
  ...
  const isFirstMessage =
    userInfo?.isFirstMessage === undefined ? true : userInfo.isFirstMessage;
  const userGreeting =
    userInfo && isFirstMessage ? `Hey ${userName}!` : 'Hey there!';
  ...
  switch (contentType) {
    case 'dailyInspiration':
      ...
      if (context === 'welcome') {
        return {
          subject:
            '🎉 Welcome to Featherweight - Your Journaling Journey Begins!',
