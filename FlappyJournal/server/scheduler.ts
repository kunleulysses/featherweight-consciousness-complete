...
async function checkAndSendDailyEmails() {
  ...
  if (currentHour === 11 && currentMinute < 15) {
    console.log(
      `🕚 It's around 11:00 AM - time to send daily inspiration emails!`
    );
    ...
    try {
      ...
      if (result.success) {
        console.log(
          `✅ Successfully sent ${result.count} daily inspiration emails`
        );
    ...
    // Not time yet
    console.log(
      `⏰ Current time: ${currentHour}:${currentMinute} - Not yet time to send daily emails (scheduled for ~11:00 AM)`
    );
  }
}
