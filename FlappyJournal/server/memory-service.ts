...
try {
  if (apiKey) {
    ...
    console.warn(
      'OpenAI API key is not configured. Memory analysis will be limited.'
    );
  }
} catch (error) {
  console.error(
    'Failed to initialize OpenAI client for memory service:',
    error
  );
  openai = null;
}
...
export const memoryService = {
  ...
  async processMessage(
    userId: number,
    message: string,
    messageType: 'email' | 'sms' | 'journal_topic' | 'conversation'
    ...
    try {
      ...
      for (const topic of analysis.topics) {
        ...
        if (matchingMemory) {
          ...
          const updatedMemory =
            await storage.incrementConversationMemoryFrequency(
              matchingMemory.id
            );
          if (updatedMemory) {
    ...
    try {
      ...
      return allMemories
        .sort((a, b) => {
          ...
          return (
            new Date(b.lastDiscussed).getTime() -
            new Date(a.lastDiscussed).getTime()
          );
        })
        .slice(0, limit);
    } catch (error) {
  ...
  async rankMemoriesByRelevance(
    ...
    try {
      ...
      try {
        ...
        const unrankedMemories = memories.filter(
          (_, index) => !validIndexes.includes(index)
        );
   ...
   */
  fallbackAnalysis(content: string): {
    topics: Array<{
      name: string;
      sentiment: string;
      importance: number;
