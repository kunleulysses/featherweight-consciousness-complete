  ...
  try {
    ...
    if (payload && payload.rawMimeBase64) {
      ...
      if (bufferString.includes('Content-Disposition: form-data')) {
        ...
        console.log(
          `🔍 Multipart data preview: ${bufferString.substring(0, 500)}...`
        );
...
async function processRawEmail(buffer: Buffer): Promise<void> {
  try {
    ...
    console.log(
      `📧 Parsed email from: ${from}, subject: ${subject}, content length: ${text.length}`
    );
...
async function processEmailFromText(text: string): Promise<void> {
  try {
    ...
    const inReplyTo =
      emailData.inReplyTo || emailData['in-reply-to'] || undefined;
