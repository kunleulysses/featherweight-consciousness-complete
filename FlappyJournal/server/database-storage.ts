...
export class DatabaseStorage implements IStorage {
      ...
      const preferencesUpdate = {
        firstName:
          profileData.firstName !== undefined
            ...
            : currentPreferences.firstName,
        lastName:
          profileData.lastName !== undefined
            ? profileData.lastName
            : currentPreferences.lastName,
        bio:
        ...
        marketingEmails:
          currentPreferences.marketingEmails !== undefined
            ? currentPreferences.marketingEmails
            : false,
        receiveInsights:
          currentPreferences.receiveInsights !== undefined
            ? currentPreferences.receiveInsights
            : true,
        theme: currentPreferences.theme || 'light',
        receiveSms:
          currentPreferences.receiveSms !== undefined
            ? currentPreferences.receiveSms
            : false,
        phoneNumber: currentPreferences.phoneNumber,
        emailDeliveryTime: currentPreferences.emailDeliveryTime || '11:00',
        disableDailyEmails:
          currentPreferences.disableDailyEmails !== undefined
            ? currentPreferences.disableDailyEmails
            : false,
      };
  ...
  async createBillingTransaction(
    transaction: InsertBillingTransaction
  ...
  async updateJournalEntry(
    id: number,
    partialEntry: Partial<InsertJournalEntry>
  ...
  async updateSmsMessage(
    id: number,
    partialMessage: Partial<InsertSmsMessage>
  ...
  async createConversationMemory(
    memory: InsertConversationMemory
  ): Promise<ConversationMemory> {
  ...
  async updateConversationMemory(
    id: number,
    updates: Partial<InsertConversationMemory>
  ): Promise<ConversationMemory | undefined> {
  ...
  async incrementConversationMemoryFrequency(
    id: number
  ): Promise<ConversationMemory | undefined> {
  ...
  async markConversationMemoryResolved(
    id: number,
    isResolved: boolean
  ): Promise<ConversationMemory | undefined> {
    const [updatedMemory] = await db
  ...
  async updateUserPasswordAndClearToken(
    userId: number,
    newPassword: string
  ): Promise<User> {
    try {
  ...
  async markEmailFailed(
    id: number,
    errorMessage: string
  ): Promise<EmailQueueItem | undefined> {
    const [updatedItem] = await db
      .update(emailQueue)
      .set({
        status: 'failed',
