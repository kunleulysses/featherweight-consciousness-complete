...
if (!isTwilioConfigured) {
  console.warn(
    'Twilio credentials not fully configured. SMS features will use mock mode.'
  );
} else {
  try {
...
export const twilioService = {
  ...
  async sendSmsMessage(
    ...
    if (!user.isPremium || !phoneNumber) {
      console.warn(
        `Cannot send SMS to user ${user.id}: not premium or no phone number`
      );
      return null;
    }
    ...
    try {
      ...
      if (this.isJournalEntryRequest(body)) {
        try {
          ...
          const journalContent = await this.extractJournalContent(
            body,
            user.id,
            from
          );
  ...
  async handleOptOut(phoneNumber: string): Promise<void> {
    try {
      ...
      const optOutMessage =
        'You have been successfully unsubscribed from Featherweight.world SMS messages. No more messages will be sent to this number. Reply START to resubscribe.';
  ...
  async handleHelp(phoneNumber: string): Promise<void> {
    try {
      const helpMessage =
        'Featherweight.world - AI wellness companion. Need support? Visit featherweight.world/support or email help@featherweight.world. Message and data rates may apply. Reply STOP to unsubscribe.';
  ...
  async handleOptIn(phoneNumber: string): Promise<void> {
    try {
      ...
      if (user) {
        ...
        const optInMessage =
          "Welcome to Featherweight.world! You're now subscribed to receive AI chat & wellness messages from Flappy. Message and data rates may apply. Message frequency varies. Reply HELP for help, STOP to unsubscribe.";
        await safeSendMessage(phoneNumber, optInMessage);
      } else {
        // No user found with this phone number
        const noUserMessage =
          'Welcome! Please visit featherweight.world to create an account and start your wellness journey with Flappy.';
  ...
  async respondToConversation(
    ...
    try {
      console.log(
        `📱 Processing SMS conversation for user ${user.id}: ${message.substring(0, 50)}...`
      );
      ...
      if (recentMessages && recentMessages.length > 0) {
        ...
        conversationHistory = recentMessages
          ...
          .map((msg) => {
            const sender =
              msg.direction === 'inbound' ? user.username || 'User' : 'Flappy';
      ...
      const memoryContext =
        memoryService.formatMemoriesForPrompt(relevantMemories);
      ...
      console.log(
        `🧠 Found ${relevantMemories.length} relevant memories for SMS response`
      );
      console.log(
        `📚 SMS conversation context length: ${fullContext.length} characters`
      );
      ...
      const responseContent = await generateFlappyContent(
        'emailConversation',
        fullContext,
        {
      ...
      console.log(
        `✅ Generated SMS response: ${smsResponse.substring(0, 50)}...`
      );
    ...
    if (lowerMessage === 'save') {
      if (userId) {
        ...
        if (recentMessages && recentMessages.length > 0) {
          ...
          return this.formatJournalContent(
            `Conversation with Flappy\n\n${recentConversation}`
          );
        } else {
          return this.formatJournalContent(
            'No recent conversation found to save.'
          );
        }
      } else if (phoneNumber) {
  ...
  generateJournalTitle(content: string): string {
    ...
    const datePattern =
      /^(Monday|Tuesday|Wednesday|Thursday|Friday|Saturday|Sunday), (January|February|March|April|May|June|July|August|September|October|November|December) \d{1,2}, \d{4}\s*/i;
