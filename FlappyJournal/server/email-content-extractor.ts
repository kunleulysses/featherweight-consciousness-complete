...
export class EmailContentExtractor {
  ...
  static extractFromMultipart(bufferString: string): {
    ...
    const envelopeMatch = bufferString.match(
    ...
    console.log(
      `🔍 Extracted - Sender: ${sender}, Subject: ${subject}, Content length: ${content.length}`
    );
  ...
  private static extractFromTextField(bufferString: string): string {
    ...
    for (const pattern of patterns) {
      ...
      if (match && match[1] && match[1].trim().length > 0) {
        ...
        console.log(
          `🔍 Strategy 1 found content: ${content.substring(0, 100)}...`
        );
        return content;
      }
    }
  ...
  private static extractFromRawEmail(bufferString: string): string {
    const rawEmailMatch = bufferString.match(
    ...
    console.log(
      `🔍 Strategy 2 analyzing raw email, length: ${rawEmail.length}`
    );
    ...
    if (quotedPrintableMatch && quotedPrintableMatch[1]) {
      ...
      console.log(
        `🔍 Strategy 2 found quoted-printable content: ${content.substring(0, 100)}...`
      );
      return content;
    }
    ...
    if (bodyLines.length > 0) {
      ...
      console.log(
        `🔍 Strategy 2 found email body content: ${content.substring(0, 100)}...`
      );
      return content;
    }
  ...
  private static extractAggressively(bufferString: string): string {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (!inEmailBody) {
        ...
        if (
          trimmed.match(/^(From:|To:|Subject:|Date:)/) ||
          (trimmed.length > 15 &&
          ...
          // Has spaces or is long
          inEmailBody = true;
          console.log(
            `🔍 Found start of email body: ${trimmed.substring(0, 50)}...`
          );
        }
      }
    ...
    if (contentLines.length > 0) {
      ...
      console.log(
        `🔍 Aggressive extraction found ${contentLines.length} content lines`
      );
