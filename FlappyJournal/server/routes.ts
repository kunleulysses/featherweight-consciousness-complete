...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post('/api/webhook-test', (req: Request, res: Response) => {
    ...
    if (req.body) {
      if (typeof req.body === 'object') {
        ...
        console.log(
          `Body sample: ${JSON.stringify(req.body).substring(0, 200)}...`
        );
      } else {
  ...
  app.post('/api/public/test-email', async (req: Request, res: Response) => {
    try {
      ...
      console.log(
        `✅ Test email queued for processing (Queue ID: ${saved.id})`
      );
    ...
    try {
      ...
      req.on('end', async () => {
        try {
          ...
          console.log(
            `Parsed email text: ${parsed.text?.substring(0, 100)}...`
          );
          ...
          const saved = await storage.enqueueEmail(queueItem);
          console.log(
            `✅ Raw MIME email queued for processing (Queue ID: ${saved.id})`
          );
          ...
          res
            .status(200)
            .send(
              `Error parsing raw MIME: ${error instanceof Error ? error.message : 'Unknown error'}`
            );
        }
      });
  ...
  app.post(
    ...
    async (req: Request, res: Response) => {
      ...
      if (Buffer.isBuffer(req.body)) {
        console.log(
          `✅ req.body is a Buffer. Length: ${req.body.length} bytes.`
        );
        ...
        try {
          console.error(
            `req.body (stringified): ${JSON.stringify(req.body).substring(0, 500)}...`
          );
        } catch (e) {
          console.error(
            `req.body could not be stringified. Raw form: ${String(req.body).substring(0, 500)}...`
          );
        }
      ...
      try {
        ...
        console.log(
          `📊 Processing raw MIME buffer of ${rawEmailBuffer.length} bytes`
        );
        ...
        console.log(
          `✅ Raw MIME email (base64) queued for processing. Queue ID: ${savedQueueItem.id}`
        );
  ...
  app.post(
    '/api/journal/upload',
    journalImageUpload.single('image'),
  ...
  app.patch('/api/user/phone', async (req: Request, res: Response) => {
    ...
    try {
      ...
      const updatedUser = await storage.updateUserPhoneNumber(
        req.user.id,
        phoneNumber
      );
    ...
    try {
      ...
      function generateMoodInsights(
        ...
        if (moodEntries.length > 0) {
          const [topMood, topCount] = moodEntries.sort(
            (a, b) => b[1] - a[1]
          )[0];
          ...
          const moodDescriptions: Record<string, string> = {
            happy:
              "You've been feeling quite positive lately! This is a great time to build on this momentum.",
            calm: 'Serenity has been your companion lately. These peaceful moments are perfect for deeper reflection.',
            neutral:
              'Your mood has been balanced recently. This equilibrium can be a good foundation for growth.',
            ...
            frustrated:
              'Frustration has been present in your recent entries. Consider what specific situations trigger these feelings.',
          ...
          insights.push({
            ...
            description:
              moodDescriptions[topMood] ||
              "We're noticing patterns in your mood entries.",
        ...
        const positiveEmotions =
          (moodFrequency['happy'] || 0) + (moodFrequency['calm'] || 0);
        const negativeEmotions =
          (moodFrequency['sad'] || 0) + (moodFrequency['frustrated'] || 0);

        if (positiveEmotions > negativeEmotions && positiveEmotions > 0) {
          insights.push({
            title: 'Positive Outlook',
            description:
              'Your entries show a tendency toward positive emotions, which can enhance resilience and creative thinking.',
            emoji: '✨',
          });
        } else if (
          negativeEmotions > positiveEmotions &&
          negativeEmotions > 0
        ) {
          insights.push({
            title: 'Emotional Processing',
            description:
              "You're processing some challenging emotions, which shows courage and self-awareness. Consider what small actions might shift your perspective.",
            emoji: '🌱',
          });
        }
        ...
        if (insights.length < 3) {
          const randomInsights = [
            {
              ...
              description:
                'Try journaling at the same time each day to build a consistent habit that sticks.',
              emoji: '⏰',
            },
            {
              title: 'Self-Discovery',
              description:
                'Looking back at old journal entries can reveal patterns and growth you might not have noticed in the moment.',
              emoji: '🔍',
            },
            {
              title: 'Journaling Habit',
              description:
                'Even short journal entries of 2-3 sentences can provide valuable insights when reviewed over time.',
              emoji: '💫',
            },
            {
              title: 'Mood Patterns',
              description:
                'Your mood often follows patterns. Tracking it can help you identify triggers and make positive adjustments.',
              emoji: '📊',
            },
          ];
          ...
          while (insights.length < 3 && randomInsights.length > 0) {
            const randomIndex = Math.floor(
              Math.random() * randomInsights.length
            );
          ...
          insights: [
            {
              ...
              description:
                'Welcome to your mood tracker! Start journaling to see patterns and insights about your emotional well-being.',
              emoji: '👋',
            },
            {
              title: 'Mood Tracking',
              description:
                'Select a mood for each journal entry to build your personal mood pattern visualization.',
              emoji: '📊',
            },
            {
              title: 'Reflection Tip',
  ...
  // Autonomous thought generation endpoint for testing
  app.post('/api/ideas', async (req: Request, res: Response) => {
    try {
      ...
      // Import AutonomousThoughtGenerator dynamically
      const { AutonomousThoughtGenerator } = await import('./autonomous-thought-generator');
      const thoughtGenerator = new AutonomousThoughtGenerator();
      
      // Generate autonomous thought
      const thought = await thoughtGenerator.generateAutonomousThought();
      
      res.json({
        concept: thought?.concept || `Generated concept for: ${prompt}`,
        insight: thought?.insight || `Philosophical insight related to ${prompt}`,
        timestamp: new Date().toISOString(),
        prompt,
        venice_integration: 'active'
      });
    } catch (error: any) {
      console.error('Error generating autonomous thought:', error);
      res.status(500).json({ 
        error: error.message,
        venice_integration: 'failed'
      });
    }
  });
}
