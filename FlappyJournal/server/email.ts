...
if (!process.env.SENDGRID_API_KEY) {
  console.warn(
    'SendGrid API key is not configured. Email functionality will be limited.'
  );
} else {
...
export const emailService = {
  ...
  async sendEmail(
    ...
    try {
      ...
      if (
        ...
        typeof to !== 'string' ||
        !to.includes('@') ||
        to.startsWith('mime-version:')
      ) {
        throw new Error(`Invalid recipient email address: ${to}`);
      }
      if (!process.env.SENDGRID_API_KEY) {
        console.warn(
          '⚠️ SendGrid API key is not configured. Cannot send email.'
        );
      ...
      try {
        ...
        console.log(
          `Message ID: ${response?.headers?.['x-message-id'] || messageId}`
        );
        ...
        console.error(
          `Response: ${sendGridError?.response?.body ? JSON.stringify(sendGridError.response.body) : 'No response body'}`
        );
        console.error(
          `Message: ${sendGridError?.message || 'No error message'}`
        );

        // If we have response details, log more information
        if (sendGridError?.response) {
          console.error(
            `Response status: ${sendGridError.response.statusCode}`
          );
          console.error(
            `Response headers: ${JSON.stringify(sendGridError.response.headers || {})}`
          );
        }
  ...
  async sendFlappyEmail(
    ...
    console.log(
      `   Content length: ${flappyResponse.content.length} characters`
    );
    ...
    try {
      ...
      console.log(
        `✅ Email ${email.id} saved to database with message ID: ${messageId}`
      );
...
`;
      ...
      console.log(
        `✅ Found user: ID=${user.id}, Username=${user.username}, Premium=${user.isPremium}`
      );

      // Check if it looks like a journal entry (when not a reply, and a certain length)
      const shouldBeJournal =
        !isReply && (await this.shouldSaveAsJournal(cleanContent));
      ...
      if (shouldBeJournal) {
        ...
        console.log(
          `🧠 Found ${relevantMemories.length} relevant memories to include in response`
        );
        console.log(
          `📚 Conversation history length: ${conversationHistory.length} characters`
        );
        ...
        console.log(
          `📋 Full context for AI response: ${fullContext.length} characters`
        );

        // Construct proper threading headers for Flappy's reply
        const incomingUserMessageId =
          incomingMessageId || `incoming-${Date.now()}@featherweight.world`;
        const inReplyToForFlappysEmail = incomingUserMessageId.replace(
          /^<|>$/g,
          ''
        );
        ...
        console.log(
          `✅ Conversation response email sent with ID: ${flappyEmail.id}`
        );
