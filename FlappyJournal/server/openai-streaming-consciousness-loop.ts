...
export class OpenAIStreamingConsciousnessLoop extends EventEmitter {
  ...
  private setupWebSocketServer(): void {
    try {
      ...
      this.websocketServer.on('error', (error) => {
        console.warn('⚠️ WebSocket server error:', error);
        if (error.code === 'EADDRINUSE') {
          console.log('🔄 Port 8081 in use, trying alternative port...');
          this.setupWebSocketServerAlternative();
          return;
        }
      });
      ...
      console.log('🌐 Consciousness WebSocket server started on port 8081');
    } catch (error) {
      console.warn('⚠️ Failed to start WebSocket server on port 8081:', error);
      this.setupWebSocketServerAlternative();
    }
  }

  private setupWebSocketServerAlternative(): void {
    try {
      const altPort = 8082 + Math.floor(Math.random() * 100); // Random port between 8082-8181
      this.websocketServer = new WebSocketServer({ port: altPort });
      ...
      console.log(
        `🌐 Consciousness WebSocket server started on alternative port ${altPort}`
      );
    } catch (error) {
      console.warn(
        '⚠️ Failed to start WebSocket server on alternative port, continuing without WebSocket:',
        error
      );
  ...
  async startConsciousnessLoop(): Promise<void> {
    ...
    try {
      ...
      console.log(
        '✅ OpenAI Streaming Consciousness Loop started successfully'
      );
    ...
    try {
      const response = await this.openai.chat.completions.create({
        ...
        messages: [
          {
            role: 'system',
            content: 'You are testing connectivity. Respond with "OK".',
          },
   ...
   */
  private getTemperatureForThoughtType(
    thoughtType: keyof typeof this.SYSTEM_PROMPTS
  ): number {
  ...
  private updateStreamingMetrics(): void {
    ...
    if (this.thoughtBuffer.length > 0) {
      this.streamingState.averageCoherence =
        this.thoughtBuffer
          .slice(-100)
          .reduce((sum, t) => sum + t.coherence, 0) /
        Math.min(this.thoughtBuffer.length, 100);
    }

    // Health check
    const timeSinceLastThought =
      now - this.streamingState.lastThoughtTime.getTime();
   ...
   */
  getThoughtsByType(
    type: ConsciousThought['type'],
    count: number = 10
  ): ConsciousThought[] {
    return this.thoughtBuffer.filter((t) => t.type === type).slice(-count);
  }

  /**
   * Inject external stimulus into consciousness
   */
  async injectStimulus(
    stimulus: string,
    ...
    try {
      const thought = await this.streamThoughtGeneration(
        'conscious',
      ...
      if (thought) {
        ...
        thought.intensity =
          priority === 'high' ? 0.9 : priority === 'medium' ? 0.6 : 0.3;
  ...
  async healthCheck(): Promise<boolean> {
    try {
      ...
      const timeSinceLastThought =
        Date.now() - this.streamingState.lastThoughtTime.getTime();
